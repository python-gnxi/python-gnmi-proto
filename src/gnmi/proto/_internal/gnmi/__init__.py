...
class CollectorStub(betterproto.ServiceStub):
    async def reconnect(self, *, target: Optional[List[str]] = None) -> "Nil":
        target = target or []
...
class GNmiBase(ServiceBase):
    ...
    async def __rpc_subscribe(self, stream: grpclib.server.Stream) -> None:
        ...
        await self._call_rpc_handler_server_stream(
            self.subscribe, stream, request_kwargs,
        )
...
class CollectorBase(ServiceBase):
    async def reconnect(self, target: Optional[List[str]]) -> "Nil":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)

    async def __rpc_reconnect(self, stream: grpclib.server.Stream) -> None:
        request = await stream.recv_message()
        ...
        response = await self.reconnect(**request_kwargs)
        await stream.send_message(response)
